...
# Evaluation for the JEPA-only LoRA SQA3D run.
# The use_jepa_only flag is persisted in the trained checkpoint's config.json,
# so the model rebuild during inference automatically takes the JEPA-only path.

...
# JEPA-only LoRA SQA3D ablation: SigLIP is bypassed; visual tokens come solely
# from the pre-extracted 3D-JEPA features via jepa_projector (256 -> hidden).
...
torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" --master_port 43000 \
    ...
    --use_jepa_only True \
    --jepa_only_projector_type transformer \
    --jepa_only_num_layers 2 \
    --jepa_only_num_heads 8 \
    --jepa_only_use_view_token True \
    --lora_enable True \
    --lora_r 128 \
    --lora_alpha 256 \
    --lora_dropout 0.05 \
